        ...
        pass
        {
            ...
            v2f VERT(a2v i)
            {
                ...
                for (int x = -sampleCounts; x <= sampleCounts; x++)
                {
                    ...
                    {
                        float2 samplePosition = pivotCS.xy + o.positionCS.xy * sampleRange * float2(x, y) /
                            singeAxisCounts; //裁剪空间的采样位置
                        ...
                        float sampleDepth = SAMPLE_TEXTURE2D_LOD(_CameraDepthTexture, sampler_CameraDepthTexture, SSuv,
                                                                 0).x; //采样当前像素点的深度值
